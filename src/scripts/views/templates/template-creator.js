...
const createRestaurantItemCardTemplate = (restaurant) => `
  <li class="card">
    <a href="/#/detail/${restaurant.id}"><div class="card-avatar">
      <picture>
        <source media="(max-width: 700px)" class="lazyload" srcset="${API_ENDPOINT.LOAD_IMAGE_SM(restaurant.pictureId)}">
...
const createRestaurantDetailTemplate = (restaurant) => {
  const cat = restaurant.categories.map((el) => el.name);
  const { foods, drinks } = restaurant.menus;
  const listFoods = foods.map((food) => `<li>${food.name}</li>`);
  const listDrinks = drinks.map((drink) => `<li>${drink.name}</li>`);
  const customerReviews = restaurant.customerReviews.map((reviewer) => {
    const { name, date, review } = reviewer;
    return `
